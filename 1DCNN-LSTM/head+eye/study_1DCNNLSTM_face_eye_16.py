...
def load_data(length, filename):
    ...
    #numpy配列を結合する。
    x = np.stack([a, b, c, d, e, f, g, h, j, k], axis=1)
...
def main():
    ...
    cnn_layer      = 1           #畳み込み層の数
    lstm_layer     = 2           #LSTM層の数
    dense_layer    = 2           #全結合層の数
    filter_size    = 2           #畳み込みフィルタのサイズ
    cnn_filters    = [40]    #畳み込み層1のフィルタ数
    lstm_units     = [64, 64]    #LSTM層1のユニット数
    dense_nodes    = [40, 16]        #全結合層1のユニット数
    dropout_rate   = 0.05        #ドロップアウト率
    ...
    for train_index, test_index in tscv.split(x, y):
     ...
     #モデル構築 
     #1D-CNN，1層目(inputsがあるため)
     p = Conv1D(filters=cnn_filters[0], kernel_size=filter_size, strides=1, padding='same', activation="relu")(inputs)
     p = LayerNormalization()(p)
     p = MaxPooling1D(pool_size=2, padding='same')(p)
     p = Dropout(dropout_rate)(p)
     #2層目
     for i in range(1, cnn_layer):
      p = Conv1D(filters=cnn_filters[i], kernel_size=filter_size, strides=1, padding='same', activation="relu")(p)
      p = LayerNormalization()(p)
      p = MaxPooling1D(pool_size=2, padding='same')(p)
      p = Dropout(dropout_rate)(p)
     ...
     #評価に用いるモデルの重みデータの保存
     model_weights = "1DCNNLSTM_face_eye_" + str(length) + "_" + str(index) + ".h5"
     ...
     print(classification_report(y_test, test_array_predicted, digits=3))
     print(classification_report(y_valid, valid_array_predicted, digits=3))
     ...
     results = []
     COLS = ['Rotation angle Vector(Head)', 'FaceEuclid', 'FaceHistgramTime', 'FaceStandardDev', 'scanFace', 
             'Eye', 'EyeEuclid', 'EyeHistgramTime', 'EyeStandardDev', 'scanGaze']
     ...
     plt.title('1DNNLSTM Feature Importance',size=16)
     ...
     df.to_csv(f'1dcnnlstm_feature_importance_{length}_{index+1}.csv',index=False)
