...
def main():
    ...
    cnn_layer      = 2           #畳み込み層の数
    dense_layer    = 3           #全結合層の数
    filter_size    = 2           #畳み込みフィルタのサイズ
    cnn_filters    = [40, 50]    #畳み込み層1のフィルタ数
    dense_nodes    = [64, 64, 32]        #全結合層1のユニット数
    ...
    for train_index, test_index in tscv.split(x, y):
     ...
     #評価に用いるモデルの重みデータの保存
     model_weights = "1DCNN_face_eye_" + str(length) + "_" + str(index) + ".h5"
     ...
     results = []
     COLS = ['Face', 'FaceEuclid', 'FaceHistgramTime', 'FaceStandardDev', 'scanFace', 
     ...
     df.to_csv(f'1dcnn_feature_importance_fold_{index+1}.csv',index=False)
